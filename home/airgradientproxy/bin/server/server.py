...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def parse_requestline(requestline: str) -> Request:
        ...
        if cmd == 'measures/current' and args != '':
            error = 'If measures/current cmd is specified, args must be empty.'
        ...
        elif cmd == '/fetch-current-record' or (cmd == '/measures/current'):
            # /measures/current is treated as /fetch-current-record
            # so as to mimic the device itself
            request_type = RequestType.FETCH_CURRENT_RECORD
        elif cmd == '/fetch-two-minute-record':
...
def serve_requests(port: int, db_file_in: str, log_in: Logger):
    ...
    global db_file
    db_file = db_file_in
    daemon = threading.Thread(name='airgradientproxy_daemon_server',
                              target=start_server,
                              args=[port])
