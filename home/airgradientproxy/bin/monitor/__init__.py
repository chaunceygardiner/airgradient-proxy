...
class Logger(object):
    ...
    def reconfigure(self, service_name: str, log_to_stdout: bool=False, debug_mode: bool=False) -> None:
        """Repoint the logger; called by airgradientproxyd once the config
        file has been read.  The shared `log` object below is mutated in place
        so every module sees the change."""
...
# The shared logger.  Logs to stdout until airgradientproxyd reconfigures it
